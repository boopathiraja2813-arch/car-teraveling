...
function openSummary(r){lastCompletedTrip=r;$('summaryFrom').textContent=r.originalFrom||r.route?.[0]||r.from;$('summaryTo').textContent=r.originalTo||r.route?.[1]||r.to;$('summaryReturn').textContent=r.returnTrip?'Yes':'No';$('summaryOutboundDistance').textContent=`${r.outboundDistance??r.distance??0} km`;$('summaryReturnDistance').textContent=`${r.returnDistance??0} km`;$('summaryDistance').textContent=`${r.distance??0} km`;$('summaryTravel').textContent=fmt(r.travelSeconds);$('summaryWaiting').textContent=fmt(r.waitingSeconds);$('summaryRate').textContent=money(r.rate);$('summaryTravelCharge').textContent=money(r.travelCharge);$('summaryWaitingCharge').textContent=money(r.waitingCharge);$('summaryCost').textContent=money(r.totalCost??r.calculatedCost);$('returnTripBtn').classList.toggle('hidden',Boolean(r.returnTrip));$('summaryModal').classList.remove('hidden')}
function closeSummary(){$('summaryModal').classList.add('hidden');lastCompletedTrip=null}
async function confirmEnd(){if(!trip.active)return;const btn=$('confirmEndBtn');btn.disabled=true;try{const d=await api('/trips/end',{method:'POST',body:JSON.stringify({rate:Math.max(1,Number($('rateInput').value)||trip.rate)})});stopGPS();clearInterval(timer);trip={active:false,waiting:false,rate:20};closeSummary();updateUI();$('fromInput').value='';$('toInput').value='';await loadHistory();toast('Trip completed and saved to history.')}catch(e){toast(e.message)}finally{btn.disabled=false}}
async function returnTrip(){if(!trip.active)return;const btn=$('returnTripBtn');btn.disabled=true;try{const d=await api('/trips/return',{method:'POST'});Object.assign(trip,d.trip,{active:true});closeSummary();updateUI();toast('Return Trip started. GPS tracking continues.')}catch(e){toast(e.message)}finally{btn.disabled=false}}
